// M3 桩接口：为 M1 单元通路提供"M3 角色"的受控激励，把 apb_slave_if 与真实
// packet_proc_core 解耦（通路隔离——M1 场景只考核 CSR/APB 行为，不引入 M3 的处理
// 时序依赖）。本接口驱动 apb_slave_if 的"M3 结果只读输入"（busy_i/done_i/
// format_ok_i/length_error_i/type_error_i/chk_error_i/res_pkt_len_i/
// res_pkt_type_i/res_payload_sum_i/res_payload_xor_i，
// spec §2.3 M1 端口表），供 M1-03（RES_* 只读通路）/M1-05（IRQ）
...
// 场景使用。
// 仅供 TB 使用，不代表 M3 的设计契约：真实 M3 = rtl/packet_proc_core.sv（已交付并
// 接入 sim/flist/rtl.f），其单元级验证走 tb_top 的 u_packet_proc_core + ppa_core_if
// 通路，集成验证走 u_ppa_top 通路。本 stub 保留是为上述通路隔离，不是替代品。
