...
按任务难度选 subagent 档位以节省 token：机械改动用低档（haiku），常规编码用中档（sonnet），架构/疑难调试与仲裁用高档（opus，agents 默认）；档位经 Agent 调用的 model 参数指定。**派单前用 skill `/dispatch` 组装任务卡**（输入清单 + 隔离自查 + 回收核对）；DE/DV 交付按各自 agent md 中的"交付汇报"固定格式回收。
...
.claude/   agents（de/dv/rev）与 skills（handover/dispatch/closeout/evidence）
```
...
- 本地软门禁（pre-commit）：`docs.py --check` —— log 块超量未归档、status/log 首块版本不同步、✅ 无证据或 .log 证据首行无复现命令、feature-matrix ✅ 无已过关联场景、缺陷单缺修复 commit/复验证据、详情页孤儿、重复 ID、spec.md 被悄改，任一触发即拦截。
- CI 硬门禁（GitHub Actions）：跑 `docs.py --check` 与 `--handover` 冒烟。仿真类硬门禁在本地：`make regress` 100% PASS 是里程碑完成的必要条件。
