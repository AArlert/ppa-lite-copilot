...
6. 收尾走 skill `/closeout`：`make bump` → 填 log 四问 + status summary → `make docs-check` → commit → **push**。
...
- 提交自包含：源码 + 测试 + 文档同一提交。
- **每次 `/closeout` 收尾 commit 后立即 `git push`**（已是用户长期授权的标准动作，无需每次再问）；push 失败（网络/冲突/认证）如实汇报，不静默跳过、不 force push。
- 只有用户要求时才建 PR。
